# Example 9.2-single-element-Fatigue
# Not in "Finite Element Analysis of Composite Materials Using Abaqus, 1st ed." 
# DDM6TM includes temp-dependent properties, fatigue single f(N), & mech. strain
# Mech. strain not used in this example
# Plate clamped on outer boundary, subjected to DT=Temp-SFT
# Using ddm6tm-2017.obj
# ModelParameters['Cycles'] = 'N' if Cycles!=1 => fatigue
# ModelParameters['Temp']   = '-156'    # end temperature 
# ModelParameters['SFT']    = ' 177'    # stress free temperature 
# If Cycles != 0, then BetaGIc & BetaGIIc can not be = 0
# ModelParameters['BetaGIc']  = '-0.204' # [1/m], mode I
# ModelParameters['BetaGIIc'] = '-0.204' # [1/m], use value of mode I, anyway for cross-ply, GII=0
# ModelParameters['Cycles']   = '10'   # number of cycles
...
import os
os.chdir(r"C:\SIMULIA\User\DDM6TM2017")# setup WorkDirectory
import time
def convert(seconds):
...
from UgenKeywordTM_UGENS import * # utilities to write UGENS parameters on Job-1.inp
session.journalOptions.setValues(replayGeometry=COORDINATE, recoverGeometry=COORDINATE)

# Enter your model parameters here 
ModelParameters = {}                        # define a dictionary data type
# Elastic properties
# For a quadratic equation: P = a + bT + cT^2 # Units [MPa] 
ModelParameters['E1a']       = '271270.586'     
...
# Thermal properties
# For a cubic equation: P = a + bT + cT^2 + dT^3  # units: microstrains 
ModelParameters['CTE1a']     = '-0.9766711'     
...
# Fracture properties 
# For a quadratic equation: P = A + BT + BT^2 # Units [kJ/m^2]   
ModelParameters['GIcA']       = '0.18187'      
ModelParameters['GIcB']       = '0.0'      
ModelParameters['GIcC']       = '0.0' 
ModelParameters['TGImax']     = '24'      # Units [Celsius]
ModelParameters['TGImin']     = '-184'    # Units [Celsius]
ModelParameters['GIIcA']      = '1'      
ModelParameters['GIIcB']      = '0.0'      
ModelParameters['GIIcC']      = '0.0' 
ModelParameters['TGIImax']    = '24'      # Units [Celsius]
ModelParameters['TGIImin']    = '-184'    # Units [Celsius]
...
# Fiber mode [MPa]
ModelParameters['F1t']        = '1900.87'  
ModelParameters['F1c']        = '441.2'
ModelParameters['F2c']        = '57.23'
ModelParameters['mWeibull']   = '8.9'         # [m]
# Thermal Fatigue mode
# BetaGIc & BetaGIIc only needed for thermal fatigue, they define the f(N) mode I and II
...
# If Cycles != 1, then BetaGIc & BetaGIIc can not be = 0
ModelParameters['Cycles']   = '10'   # number of cycles
ModelParameters['BetaGIc']  = '-0.204' # [1/m], mode I
ModelParameters['BetaGIIc'] = '-0.204' # [1/m], use value of mode I, anyway for cross-ply, GII=0
# LSS
ModelParameters['Lss']      = [(0,1), (90,1), (0,1), (90,1)] # a [list] of tuples (angle,thickness) 1/2 symm. laminate
...
# Defined UgensKeyword 
# import/define GetKeywordPosition after 'mdb' name is defined
...
# Material is only used for later locating the KeywordBlock 
# Material properties
Material = mdb.models['Model-1'].Material(name='Material-1')
#Properties = Material.UserMaterial(mechanicalConstants= MechanicalConstants(ModelParameters))
...
# symm top Y
# Model.YsymmBC(createStepName='Initial', name='YFixed', 
              # region=Region(edges=Instance.edges.findAt(((ShellDimensionX/2., ShellDimensionX, 0.0), ), )))
...
Model.keywordBlock.replace(position, '*Shell General Section, elset=WholeLaminate, USER, PROPERTIES='+str(PROPERTIES)+', VARIABLES='
  +str(VARIABLES)+'\n'+str(WholeLaminateThickness)+', '+str(IntegrationPoints))
...
print convert(elapsed)#display the time leapsed

# post-processing 
# a = mdb.models['Model-1'].rootAssembly
# session.viewports['Viewport: 1'].setValues(displayedObject=a)
# o3 = session.openOdb(name='C:/SIMULIA/User/DDM6TM/Job-1.odb')
# session.viewports['Viewport: 1'].setValues(displayedObject=o3)
# session.viewports['Viewport: 1'].view.setValues(session.views['Front'])
# odb = session.odbs['C:/SIMULIA/User/DDM6TM/Job-1.odb']
...
# session.viewports['Viewport: 1'].odbDisplay.setPrimaryVariable(variableLabel='SDV13', outputPosition=INTEGRATION_POINT, )
# session.viewports['Viewport: 1'].odbDisplay.setPrimaryVariable(variableLabel='SDV25', outputPosition=INTEGRATION_POINT, )
# session.viewports['Viewport: 1'].odbDisplay.setPrimaryVariable(variableLabel='SDV37', outputPosition=INTEGRATION_POINT, )
#
NL = len(ModelParameters['Lss'])
ofile = open('Job-1.csv', 'wb')
Writer = csv.writer(ofile)
Header = ['Life']
for i in range(NL):
    Header.append('Crack density '+str(i+1)+' [1/mm]')

Writer.writerow(Header)
#
Results = openOdb(path='Job-1.odb', readOnly=True)#avoid warning
...
for i in range(NL):
    cd.append(frame.fieldOutputs['SDV'+str(12*i+1)].values[0].dataDouble)#2017 dataDouble
