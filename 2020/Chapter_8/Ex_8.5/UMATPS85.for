...
!     Copyright (2007,2012,2021) Ever J. Barbero. Abaqus Ex. 8.5
...
!                      of the increment. Must be updated before exit
! -----------------------------------------------------------
...
!       Recover state variables
        ...
        D2    = statev(2)
        D6    = statev(3)  
!       Get the elastic properties and model parameters
        E1      = props(1)
        E2      = props(2)
        nu12    = props(3)
        ...
        nu21    = nu12*E2/E1
        F2t     = props(7)   ! Transverse tensile strength 
        F6      = props(8)   ! In-plane shear strength
        GIc     = props(9)   ! ERR mode I
        GIIc    = props(10)  ! ERR mode II
        c1      = props(11)  ! hardening function
        c2      = props(12)  ! hardening function
        gamma0  = props(13)  ! hardening function
...
!           calculate secant stiffness 2D (plane stress) 
!           as a function of current damage
            C      = 0
...
!!           calculate the Cauchy stress using the current secant stiffness 
            ...
            sigma2 = STRESS(2)
            sigma6 = STRESS(3)
!           calculate thermodynamic loads Y1,Y2,Y6 as f. of Cauchy stresses
            Y(1) = 0
            Y(2) = sigma2 ** 2 / (1 - D2) ** 3 / E2 - nu12 / E1 /       &
     &              (1 - D2)** 2 * sigma1 * sigma2
            Y(3) = sigma6 ** 2 / (1 - D6) ** 3 / 2 / G12 ! ERRATA 2007
            Y2s = Y(2)
            Y6s = Y(3)
!           evalutate gamma
...
!               compute dg_dY*dY_dD*df_dY
                prod = dble(1 / ((1 - GIc / GIIc) * Y2s * E2 / F2t ** 2 &
     &          + GIc / GIIc * Y2s ** 2 * E2 ** 2 / F2t ** 4 + Y6s ** 2 &
     &          * G12 ** 2 / F6 ** 4) * ((1 - GIc / GIIc) * E2 / F2t ** &
     &          2 + 2 * GIc / GIIc * Y2s * E2 ** 2 / F2t ** 4) ** 2 *   &
     &          (-nu12 ** 2 * sigma2 / E1 / (-1 + D2) ** 2 * E2 / (nu21 &
     &          * nu12 - 1) * eps2 + (2 * sigma2 / (1 - D2) ** 3 / E2 - &
     &          nu12 / E1 / (1 - D2) ** 2 * sigma1) * (E1 * nu21 /      &
     &          (nu21 * nu12 - 1) * eps1 - 2 * (-1 + D2) * E2 / (nu21 * &
     &          nu12 - 1) * eps2))) / 0.4D1 + dble(32 / ((1 - GIc /     &
     &          GIIc) * Y2s * E2 / F2t ** 2 + GIc / GIIc * Y2s ** 2 *   &
     &          E2 ** 2 / F2t ** 4 + Y6s ** 2 * G12 ** 2 / F6 ** 4) *   &
     &          Y6s ** 2 * G12 ** 4 / F6 ** 8 * sigma6 / (1 - D6) ** 3  &
     &          * (-1 + D6) * eps6)
...
!               compute df_dY
                df_dY(2) = (((1 - GIc / GIIc) * Y2s * E2 / F2t ** 2 +   &
     &          GIc / GIIc * Y2s ** 2 * E2 ** 2 / F2t ** 4 + Y6s ** 2 * &
     &          G12 ** 2 / F6 ** 4) ** (-0.1D1 / 0.2D1) * ((1 - GIc /   &
     &          GIIc) * E2 / F2t ** 2 + 2 * GIc / GIIc * Y2s * E2 ** 2  &
     &          / F2t ** 4)) / 0.2D1

                df_dY(3) = ((1 - GIc / GIIc) * Y2s * E2 / F2t ** 2 +    &
     &          GIc / GIIc * Y2s ** 2 * E2 ** 2 / F2t ** 4 + Y6s ** 2 * &
     &          G12 ** 2 / F6 ** 4) ** (-0.1D1 / 0.2D1) * Y6s *         &
     &          G12 ** 2 / F6 ** 4

!               update the state variables
                delta = delta - lambda       ! hardening variable
                D2 = D2 + lambda * df_dY(2)  ! transverse damage
...
!       STEP 8. compute tangent stiffness matrix and and out-of-plane stress values
...
!       inplane stresses computed inside the RMA are current 
!       and out-of-plane values are updated in the previous step
!       STRESS = STRESS + MATMUL(DDSDDE,DSTRAN) !Alternative computation not used
