...
c *** get Properties, and Damage model parameters
...
c *** calculate D and secant stiffness 1D 
	D		= 1 - dexp(-delta_alpha*gamma_hat**m)
	C		= dexp(-delta_alpha*gamma_hat**m)*E

c *** calculate the apparent stress *** output: S11
...
!     CALCULATE THE TANGENT STIFFNESS
     ...
     &	    	      *dexp(-delta_alpha*gamma_hat**m)*E 
      ...
      ELSEIF (NDI.EQ.2) THEN
        ...
        call xit
!       left over from Ex. 3.14 for later generalizing damage routine to 2D        
!       PR21	= PR12*E2/E1
      ...
      ELSEIF (NDI.EQ.3) THEN
        ...
        call xit
!       left over from Ex. 7.5 for later generalizing damage routine to 2D        
...
!        ENDDO
!      ENDDO
c     STRESS(1)= STRESS(1) + DDSDDE(1,1)*DSTRAN(1)  ! alternative to compute stress
